...
public class OrderService {
    ...
    public OrderDto placeOrder(PlaceOrderRequest request) {
        ...
        List<StockRequest> stockReductions = new ArrayList<>();
        double total = 0;

        for (PlaceOrderRequest.OrderItemRequest itemReq : request.getItems()) {
            ProductInfo product = productClient.getProduct(itemReq.getProductId());
            ...
            stockReductions.add(new StockRequest(itemReq.getProductId(), itemReq.getQuantity()));
        }
        ...
        // Reduce stock before saving — if this fails, the order is not persisted
    ...
    public OrderDto cancelOrder(String id) {
        ...
        productClient.restoreStock(buildStockRequests(order));
    ...
    private List<StockRequest> buildStockRequests(Order order) {
        return order.getItems().stream()
                .map(item -> new StockRequest(item.getProductId(), item.getQuantity()))
                .toList();
    }
