...
class GlobalExceptionHandlerTest {
    ...
    void handleAppException_notFound_returns404() {
        AppException ex = new AppException("Order not found", HttpStatus.NOT_FOUND);
        ...
        assertThat(response.getBody()).containsEntry("message", "Order not found");
    }
    ...
    void handleAppException_forbidden_returns403() {
        AppException ex = new AppException("You do not own this order", HttpStatus.FORBIDDEN);
        ResponseEntity<Map<String, Object>> response = handler.handleAppException(ex);
