...
class OrderServiceTest {
    ...
    @Test
    void getMyOrders_withoutStatus_returnsAll() {
        Order order = Order.builder()
                .id("order-1").buyerId(BUYER_ID).items(List.of())
                .total(10.0).status(OrderStatus.PLACED).build();
    ...
    @Test
    void getMyOrders_invalidStatus_throws() {
        assertThatThrownBy(() -> orderService.getMyOrders("BOGUS"))
                .isInstanceOf(AppException.class)
                .hasMessageContaining("Invalid status");
    }

    @Test
    void getOrderById_buyerCanAccess() {
        Order order = Order.builder()
                .id("order-1").buyerId(BUYER_ID).items(List.of())
                .total(10.0).status(OrderStatus.PLACED).build();
    ...
    void getOrderById_accessDenied_throws() {
        Order order = Order.builder()
                .id("order-1").buyerId("other-buyer")
                .items(List.of(OrderItem.builder().productId("p1").sellerId("other-seller").build()))
                .total(10.0).status(OrderStatus.PLACED).build();
    ...
    void deleteOrder_placed_success() {
        Order order = Order.builder()
                .id("order-1").buyerId(BUYER_ID)
                .items(List.of(OrderItem.builder().productId("prod-1").quantity(2).subtotal(20.0).build()))
                .total(20.0).status(OrderStatus.PLACED).build();
        ...
        assertThatCode(() -> orderService.deleteOrder("order-1")).doesNotThrowAnyException();
        verify(orderRepository).deleteById("order-1");
    }

    @Test
    void redoOrder_notCancelledOrDelivered_throws() {
        Order order = Order.builder()
                .id("order-1").buyerId(BUYER_ID).items(List.of())
                .total(10.0).status(OrderStatus.SHIPPED).build();
        ...
        assertThatThrownBy(() -> orderService.redoOrder("order-1"))
                .isInstanceOf(AppException.class)
                .hasMessageContaining("CANCELLED or DELIVERED");
    }

    @Test
    void advanceOrderStatus_placed_toConfirmed() {
        var auth = new UsernamePasswordAuthenticationToken(
        ...
        Order order = Order.builder()
                .id("order-1").buyerId(BUYER_ID)
                .items(List.of(OrderItem.builder()
                        .productId("prod-1").sellerId(SELLER_ID).quantity(1).subtotal(10.0).build()))
                .total(10.0).status(OrderStatus.PLACED).build();

        when(orderRepository.findById("order-1")).thenReturn(Optional.of(order));
        when(orderRepository.save(any())).thenAnswer(i -> i.getArgument(0));

        OrderDto result = orderService.advanceOrderStatus("order-1");

        assertThat(result.getStatus()).isEqualTo("CONFIRMED");
    }

    @Test
    void advanceOrderStatus_delivered_cannotAdvance_throws() {
        var auth = new UsernamePasswordAuthenticationToken(
                SELLER_ID, null, List.of(new SimpleGrantedAuthority("ROLE_SELLER")));
        SecurityContextHolder.getContext().setAuthentication(auth);
        ...
        assertThatThrownBy(() -> orderService.advanceOrderStatus("order-1"))
                .isInstanceOf(AppException.class)
                .hasMessageContaining("cannot be advanced");
    }

    @Test
    void advanceOrderStatus_notSeller_throws() {
        var auth = new UsernamePasswordAuthenticationToken(
                SELLER_ID, null, List.of(new SimpleGrantedAuthority("ROLE_SELLER")));
        SecurityContextHolder.getContext().setAuthentication(auth);
        ...
        Order order = Order.builder()
                .id("order-1").buyerId(BUYER_ID)
                .items(List.of(OrderItem.builder()
                        .productId("prod-1").sellerId("other-seller").quantity(1).subtotal(10.0).build()))
                .total(10.0).status(OrderStatus.PLACED).build();
    ...
    @Test
    void getUserAnalytics_calculatesCorrectly() {
        Order order = Order.builder()
                .id("order-1").buyerId(BUYER_ID)
                .items(List.of(
                        OrderItem.builder().productId("p1").productName("Widget")
                                .category("Electronics").quantity(2).subtotal(20.0).build(),
                        OrderItem.builder().productId("p2").productName("Gadget")
                                .category("Electronics").quantity(1).subtotal(15.0).build()
                ))
                .total(35.0).status(OrderStatus.PLACED).build();

        when(orderRepository.findByBuyerIdOrderByCreatedAtDesc(BUYER_ID)).thenReturn(List.of(order));

        UserAnalyticsDto result = orderService.getUserAnalytics();

        assertThat(result.getTotalSpent()).isEqualTo(35.0);
        assertThat(result.getTotalOrders()).isEqualTo(1);
        assertThat(result.getMostBoughtProducts()).hasSize(2);
        assertThat(result.getTopCategories()).hasSize(1);
        assertThat(result.getTopCategories().get(0).getCategory()).isEqualTo("Electronics");
    }

    @Test
    void getUserAnalytics_skipsCancelledOrders() {
        Order cancelled = Order.builder()
                .id("order-1").buyerId(BUYER_ID)
                .items(List.of(OrderItem.builder().productId("p1").productName("Widget")
                        .category("Electronics").quantity(1).subtotal(10.0).build()))
                .total(10.0).status(OrderStatus.CANCELLED).build();
        ...
        assertThat(result.getMostBoughtProducts()).isEmpty();
    }

    @Test
    void getSellerAnalytics_calculatesCorrectly() {
        var auth = new UsernamePasswordAuthenticationToken(
                SELLER_ID, null, List.of(new SimpleGrantedAuthority("ROLE_SELLER")));
        SecurityContextHolder.getContext().setAuthentication(auth);
        ...
        Order order = Order.builder()
                ...
                .items(List.of(
                        ...
                        OrderItem.builder().productId("p2").productName("Other")
                                .sellerId("other-seller").quantity(1).subtotal(10.0).build()
                ))
                .total(40.0).status(OrderStatus.PLACED).build();
    ...
    @Test
    void getSellerAnalytics_skipsCancelledOrders() {
        var auth = new UsernamePasswordAuthenticationToken(
                SELLER_ID, null, List.of(new SimpleGrantedAuthority("ROLE_SELLER")));
        SecurityContextHolder.getContext().setAuthentication(auth);
        ...
        Order cancelled = Order.builder()
                .id("order-1").buyerId(BUYER_ID)
                .items(List.of(OrderItem.builder().productId("p1").productName("Widget")
                        .sellerId(SELLER_ID).quantity(2).subtotal(20.0).build()))
                .total(20.0).status(OrderStatus.CANCELLED).build();

        when(orderRepository.findOrdersContainingSeller(SELLER_ID)).thenReturn(List.of(cancelled));

        SellerAnalyticsDto result = orderService.getSellerAnalytics();

        assertThat(result.getTotalRevenue()).isEqualTo(0.0);
        assertThat(result.getTotalOrdersProcessed()).isEqualTo(0);
    }
}
